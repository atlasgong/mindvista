...
export function DirectoryItemBox({ item, type }: DirectoryItemBoxProps) {
    return (
        <Link href={`/directory/${type}/${item.slug}`} key={item.id} className="group block rounded-lg border border-cBorder bg-cBackgroundOffset p-6 shadow-sm transition-all duration-200 hover:border-blue-200 hover:shadow-lg hover:shadow-blue-50 dark:hover:border-blue-800 dark:hover:shadow-blue-950/50">
