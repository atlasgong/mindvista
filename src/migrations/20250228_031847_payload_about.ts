...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum__legal_v_published_locale" AS ENUM('en', 'fr');
  ...
  CREATE TYPE "public"."enum__about_v_published_locale" AS ENUM('en', 'fr');
  ...
  CREATE TYPE "public"."enum__sponsor_v_published_locale" AS ENUM('en', 'fr');
  ...
  CREATE TABLE IF NOT EXISTS "_sponsor_v" (
  	...
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"snapshot" boolean,
  	"published_locale" "enum__sponsor_v_published_locale",
  	"latest" boolean
  );
  ...
  CREATE INDEX IF NOT EXISTS "_legal_v_snapshot_idx" ON "_legal_v" USING btree ("snapshot");
  CREATE INDEX IF NOT EXISTS "_legal_v_published_locale_idx" ON "_legal_v" USING btree ("published_locale");
  ...
  CREATE INDEX IF NOT EXISTS "_about_v_snapshot_idx" ON "_about_v" USING btree ("snapshot");
  CREATE INDEX IF NOT EXISTS "_about_v_published_locale_idx" ON "_about_v" USING btree ("published_locale");
